...
[proj-vid]: ./output_videos/project_video.mp4 (Project Video Output)
...
Although the reduced search area fixed the false positives issue, this fix seems to be a bit of a hack. Since our project test video has the car only on the farthest left lane for the entirety of the video, we can assume that most likely no cars are to be found to the left of our path of travel. However, in a live situation, should the car move to a lane on the right, then this search will be missing any cars that appear to the left of our vehicle point of view. This solution does not generalize well. A better way to get rid of the false positives should be investigated.
